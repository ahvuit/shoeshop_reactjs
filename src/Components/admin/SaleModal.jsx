...
const SaleModal = (props) => {
  ...
  const handleSubmit = (values) => {
    if(action==='edit'){
      const eSale={...values,banner:image,startDay: startDay.format('YYYY-MM-DD'), endDay:endDay.format('YYYY-MM-DD'),createdDate: sale.createdDate};
      console.log(eSale);
      const {salesId,...rest}= eSale
      
    dispatch(updateSale(salesId, eSale))
        .then(() => {
          setOpenModal(false);
          msg.success("Update sale successful");
        })
        .catch(() => {});
    }else if (action==='add') {
      const iSale={...values,banner:image,salesId: null,startDay: startDay.format('YYYY-MM-DD'), endDay:endDay.format('YYYY-MM-DD'),createdDate: null};
      console.log(iSale);
      dispatch(insertSale(iSale))
        .then(() => {
          setOpenModal(false);
          msg.success("Insert sale successful");
        })
        .catch(() => {});
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Form.Item name="salesId" label="Sale Id">
            <Input disabled />
          </Form.Item>
          <Form.Item
            name="salesName"
